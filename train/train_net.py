...
def train_net(net, train_path, num_classes, batch_size,
              ...
              momentum, weight_decay, lr_refactor_step, lr_refactor_ratio, gpus,
              freeze_layer_pattern='stage',
              num_example=10000, label_pad_width=350,
              nms_thresh=0.45, force_nms=False, ovp_thresh=0.5,
              use_difficult=False, class_names=None,
              voc07_metric=True, nms_topk=400, force_suppress=False,
    ...
    #train_iter = DetRecordIter(val_path, batch_size, data_shape, mean_pixels=mean_pixels,
                #label_pad_width=label_pad_width, path_imglist=train_list, **cfg.train)
    if val_path:
        #val_iter = DetRecordIter(val_path, batch_size, data_shape, mean_pixels=mean_pixels,
                                   #label_pad_width=label_pad_width, path_imglist=train_list, **cfg.valid)
        val_iter = DetRecordIter(val_path, batch_size, data_shape, mean_pixels=mean_pixels,
            label_pad_width=label_pad_width, path_imglist=val_list, **cfg.valid)
    else:
        val_iter = None
    # load symbol
    net = get_symbol_train(net, data_shape[1], batch_size, gpus, num_classes=num_classes,
    ...
    learning_rate, lr_scheduler = get_lr_scheduler(learning_rate, lr_refactor_step, lr_refactor_ratio, num_example, batch_size, begin_epoch)
    ...
    if voc07_metric:
        valid_metric = VOC07MApMetric(ovp_thresh, use_difficult, class_names, pred_idx=6)
    else:
        valid_metric = MApMetric(ovp_thresh, use_difficult, class_names, pred_idx=6)
